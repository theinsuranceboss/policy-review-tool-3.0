...
const Header: React.FC<HeaderProps> = ({ isAdmin, setIsAdmin, onOpenWizard, onGoHome }) => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-black/30 backdrop-blur-2xl border-b border-white/5 px-4 md:px-8 py-6 flex items-center justify-between">
        ...
        >
          ...
          <div className="hidden sm:block text-left">
            <div className="flex items-center gap-3">
              <h1 className="font-black text-xl md:text-2xl tracking-tight leading-none">The Insurance Boss</h1>
            </div>
            <p className="text-[9px] text-gray-500 font-black tracking-widest uppercase mt-1">Authority audit</p>
          </div>
        </div>

        {/* PROMINENT HOME BUTTON NEXT TO LOGO */}
        <button 
          onClick={onGoHome}
          className="flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-white/5 hover:bg-yellow-400 hover:text-black text-[10px] font-black tracking-wider transition-all border border-white/5 group shadow-xl uppercase"
        >
          <svg className="w-4 h-4 text-yellow-400 group-hover:text-black transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <nav className="flex items-center gap-4 md:gap-8">
        {isAdmin ? (
          <div className="flex items-center gap-4 md:gap-6">
             <Link to="/admin" className={`text-xs font-black tracking-wider transition-colors uppercase ${location.pathname === '/admin' ? 'text-yellow-400' : 'text-gray-500 hover:text-white'}`}>
              Admin vault
            </Link>
            <button 
              onClick={handleLogout}
              className="flex items-center gap-2 px-4 md:px-6 py-3 rounded-2xl bg-red-500/5 hover:bg-red-500/10 text-red-400 text-xs font-black tracking-wider transition-all border border-red-500/10 uppercase"
            >
              Logout
            </button>
          </div>
