...
const AdminLogin: React.FC<AdminLoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Updated credentials as requested: theinsuranceboss / admin
  ...
  return (
    <div className="max-w-md mx-auto py-24 space-y-8 animate-in fade-in zoom-in-95 duration-500 text-center">
      ...
      <form onSubmit={handleSubmit} className="bg-[#1a1a1a]/40 backdrop-blur-xl p-8 rounded-3xl border border-white/5 space-y-6 text-left shadow-2xl">
        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-500 tracking-widest px-1">Username</label>
          <input 
            type="text" 
            value={username}
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-500 tracking-widest px-1">Password</label>
          <input 
            type="password" 
            value={password}
