...
const WizardForm: React.FC<WizardFormProps> = ({ onSubmit, onCancel }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 pb-20 text-left">
      ...
      <div className="glass-card rounded-[2.5rem] p-8 md:p-12 min-h-[500px] flex flex-col shadow-2xl">
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 text-left">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <p className="text-[10px] font-bold text-gray-500 tracking-widest uppercase">Business</p>
                <p className="font-bold text-white">{formData.businessName}</p>
              </div>
              <div className="space-y-1">
                <p className="text-[10px] font-bold text-gray-500 tracking-widest uppercase">Contact</p>
                <p className="font-bold text-white">{formData.contactName}</p>
              </div>
            </div>
            {formData.industries && formData.industries.length > 0 && (
              <div className="space-y-1">
                <p className="text-[10px] font-bold text-gray-500 tracking-widest uppercase">Industries</p>
        ...
        <div className="mt-auto pt-10 flex gap-4">
          {step > 1 && (
            <button 
              onClick={handleBack}
              className="px-8 py-4 rounded-2xl border border-white/10 hover:bg-white/5 transition-all text-sm font-bold uppercase"
            >
              Back
            </button>
          )}
          <button 
            ...
            className="flex-1 bg-yellow-400 text-black py-4 rounded-2xl font-black tracking-wider hover:bg-yellow-500 transition-all flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50 shadow-lg uppercase"
          >
            {isSubmitting ? (
      ...
      <button 
        onClick={onCancel}
        className="w-full text-center text-gray-500 text-xs hover:text-white transition-colors tracking-widest uppercase"
      >
        Cancel submission
      </button>
    </div>
...
const Input: React.FC<{ label: string; required?: boolean; type?: string; value?: string; disabled?: boolean; onChange?: (v: string) => void }> = ({ label, required, type = 'text', value, disabled, onChange }) => (
  <div className="space-y-1.5 group">
    <label className="text-[10px] font-black text-gray-500 tracking-widest group-focus-within:text-yellow-400 transition-colors uppercase">
...
const ToggleRow: React.FC<{ label: string; value: boolean; onChange: (v: boolean) => void }> = ({ label, value, onChange }) => (
  <div className="flex items-center justify-between p-6 bg-white/5 rounded-3xl border border-white/5 text-left">
    <span className="font-bold text-gray-300">{label}</span>
    <div className="flex gap-2">
      <button 
        onClick={() => onChange(true)}
        className={`px-5 py-2 rounded-xl text-xs font-black transition-all uppercase ${value ? 'bg-yellow-400 text-black' : 'bg-white/5 text-gray-500 hover:text-white'}`}
      >
        Yes
      </button>
      <button 
        onClick={() => onChange(false)}
        className={`px-5 py-2 rounded-xl text-xs font-black transition-all uppercase ${!value ? 'bg-white/20 text-white' : 'bg-white/5 text-gray-500 hover:text-white'}`}
      >
        No
      </button>
    </div>
